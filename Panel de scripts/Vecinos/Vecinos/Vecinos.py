...
# =========================
def solicitar_semana_analisis():
    """
    Solicita al usuario qué semana desea analizar.
    """
    hoy = datetime.today()
    mes_actual = hoy.month
    anio_actual = hoy.year
    
    print(f"\n📅 Mes actual: {month_name[mes_actual]} {anio_actual}")
    print("Seleccione la semana a analizar (basada en miércoles y jueves):")
    print("0. Días antes de la primera semana de miércoles/jueves")
    
    # Mostrar las semanas disponibles con sus fechas
    for i in range(1, 6):
        try:
            miercoles, jueves = obtener_rango_semana_miercoles_jueves(mes_actual, anio_actual, i)
            print(f"{i}. Semana {i}: {miercoles.strftime('%d/%m')} (mié) - {jueves.strftime('%d/%m')} (jue)")
        except:
            print(f"{i}. Semana {i}: No disponible")
    
    while True:
        try:
            opcion = int(input("\nIngrese el número de semana (0-5): "))
            if 0 <= opcion <= 5:
                return opcion, mes_actual, anio_actual
            else:
                print("Por favor, ingrese un número entre 0 and 5")
        except ValueError:
            print("Por favor, ingrese un número válido")

# Obtener la semana a analizar
semana_analizar, mes_analizar, anio_analizar = solicitar_semana_analisis()
fecha_inicio, fecha_fin = obtener_rango_semana_miercoles_jueves(mes_analizar, anio_analizar, semana_analizar)

print(f"\n📊 Analizando semana {semana_analizar}: {fecha_inicio.strftime('%d/%m/%Y')} (miércoles) - {fecha_fin.strftime('%d/%m/%Y')} (jueves)")
